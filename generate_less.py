...
def main():
    ...
    # Load models
    # Load to CPU first to avoid GPU OOM during initialization (~40GB total model size)
    # enable_vram_management will then move only the needed portions to GPU
    ...
    # Partial DiT offload: keep 8B params persistent on GPU (~16GB), offload remaining 6B to CPU
    # GPU peak: ~16GB (DiT) + 1GB (VAE) + 0.3GB (cam) + 4GB (activations) ≈ 21GB
    # Increase num_persistent_param_in_dit for faster inference (uses more VRAM)
    # Decrease it to save more VRAM (slower inference, more CPU<->GPU transfers)
    pipe.enable_vram_management(num_persistent_param_in_dit=8_000_000_000)
