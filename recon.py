...
DEPTHCRAFTER_ROOT = REPO_ROOT / "DepthCrafter"    
...
def main():
    ...
    parser.add_argument("--da3_model_dir", type=str, default=str(DEFAULT_DA3_MODEL_DIR), help="DA3 model directory")
    parser.add_argument("--target_n_frames", type=int, default=49, help="Target number of frames")
    parser.add_argument("--max_res", type=int, default=320, help="Maximum resolution of input frames")
    parser.add_argument("--da3_process_res", type=int, default=504, help="DA3 processing resolution")
    parser.add_argument("--rayhead", action="store_true", default=False, help="Enable the DA3 ray head pose")
    parser.add_argument("--dc_steps", type=int, default=5, help="Number of DepthCrafter inference steps")
    parser.add_argument("--view_type", type=str, default="front", choices=["front", "left", "right", "angle"], help="Fixed viewpoint for rendering")
    parser.add_argument("--angle", type=float, default=30.0, help="Fixed viewpoint angle")
    parser.add_argument("--zoom_out_factor", type=float, default=1.3, help="Zoom-out factor")
    parser.add_argument("--min_angle_deg", type=float, default=3.0, help="Minimum mesh face angle")
    parser.add_argument("--edge_threshold", type=float, default=0.0008, help="Mesh edge threshold")
    parser.add_argument("--edge_depth_value", type=float, default=100.0, help="Boundary depth value")
    parser.add_argument("--mask_threshold", type=int, default=125, help="Mask threshold")
    parser.add_argument("--depth_offset", type=float, default=0.15, help="Rendering radius offset")
    parser.add_argument("--save_mesh", action="store_true", help="Also save the mesh from the first frame")
